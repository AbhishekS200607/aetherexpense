/**
 * AetherExpense — Ethos Edit Transaction Screen
 *
 * Implements Stitch `add_transaction` design layout for editing an existing transaction:
 *   - Pre-fills form with existing SQLite transaction record
 *   - Large centered amount input
 *   - Segmented control (EXPENSE / INCOME)
 *   - Category selection grid
 *   - Underlined metadata inputs (Payment Method, Date & Time, Merchant, Note)
 *   - Full-width Update CTA button
 */
...
export default function EditTransactionScreen() {
  ...
  // Load existing transaction from SQLite
  useEffect(() => {
    if (!id) return;
    async function load() {
      try {
        ...
        if (txn) {
          const typeVal = txn.type as TransactionType;
          setActiveType(typeVal);
          setSelectedAccountId(txn.account_id ?? '');
          reset({
            type: typeVal,
            amount: String((txn.amount / 100).toFixed(2)),
        ...
        console.error('[EditTransactionScreen] Load error:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Reload categories and active accounts when transaction type changes
  useEffect(() => {
  ...
  const onSubmit = async (data: TransactionFormSchema) => {
    ...
    try {
      ...
      console.error('[EditTransactionScreen] Update error:', err);
